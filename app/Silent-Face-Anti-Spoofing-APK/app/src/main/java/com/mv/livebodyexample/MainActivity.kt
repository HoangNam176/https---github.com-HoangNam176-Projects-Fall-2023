...
class MainActivity : AppCompatActivity(), SetThresholdDialogFragment.ThresholdDialogListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (hasPermissions()) {
            // init successful => Read information from camera (front camera and back camera)
            init()
        } else {
            // request permission => Show display in Phone (Allow or Deny)
            requestPermission()
        }
                    ...
                    if (enginePrepared && data != null) {
                        if (!working) {
                            GlobalScope.launch(detectionContext) {
                                working = true
                                // results include Face Detector + Classification Real / Fake
